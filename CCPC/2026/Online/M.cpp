...
void solve() {
    ...
    while (L < R) {
        ...
        if (L == R)
            break;
        if (a[L] + a[R] == 2 * k) {
            (ans += f(s1[L], s2[R]) - f(s1[L - 1], s2[R]) -
                    f(s1[L], s2[R + 1]) + f(s1[L - 1], s2[R + 1])) %= p;
        }
        // cout << a[L] << " " << a[R] << " " << ans << "+++\n";
        L++;
    }
    if (a[L] == k) {
        // (ans += f(s1[L - 1], s2[L + 1]) * (ksm(2, b[L] - 1) - 1 + p) % p) %=
        // p; cout << f(s1[L - 1], s2[L + 1]) * (ksm(2, b[L] - 1) - 1 + p) % p
        //      << "!!\n";
        for (int i = 1; i < b[L]; i++) {
            (ans += f2(s1[L - 1] + i - 1, s2[L + 1] + b[L] - i) -
                    f2(s1[L - 1] + i - 1, s2[L + 1])) %= p;
        }
        for (int i = 1; i <= b[L]; i += 2) {
        }
        for (int i = 1; i <= b[L]; i++) {
